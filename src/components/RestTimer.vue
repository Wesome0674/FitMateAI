<template>
  <div
    class="fixed bg-[#0A2540] w-full z-[500] bottom-0 slideIn rounded-t-[16px] px-[30px] py-[20px]"
  >
    <img
      @click="$emit('toggle')"
      class="absolute top-[20px] left-[20px]"
      src="@/assets/img/svg/x-mark.svg"
      alt="rest timer"
    />
    <div class="flex flex-col h-full gap-[25px]">
      <div class="flex flex-col items-center gap-[5px]">
        ...
        <Typography variant="h5" theme="badge" weight="light">{{
          exerciceName
        }}</Typography>
        <div
          class="border border-dashed border-[#f4f4f5]/85 h-[1px] w-full"
        ></div>
      </div>
      <div class="flex flex-col items-center gap-[25px]">
        <div class="flex gap-[25px] items-end">
          <Typography
            ...
            >-15s</Typography
          >
          <Typography variant="subtitle" theme="primary">{{
            formattedTime
          }}</Typography>
          <Typography
            variant="h4"
        ...
        <Button
          ...
          @click="startTimer"
          :icon="arrowRight"
          :label="timerRunning ? 'Stop Timer' : 'Set Timer'"
          :primary="false"
          size="large"
        />
      </div>
