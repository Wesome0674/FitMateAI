<template>
    <div class="pb-[100px] box space-y-[30px] ">
      <div class="space-y-[10px]">
        ...
        <div class="w-full gap-[8px] flex items-center py-[10px] px-[15px] border-b border-b-[#E2E8F0]">
          <input class=" w-full border-none outline-none text-[#425466]" placeholder="Search for an exercise" v-model="routineName" />
        </div>
      </div>
      ...
      <div class="space-y-[20px]">
        ...
        <div class="w-full gap-[8px] flex items-center py-[10px] px-[15px] rounded-[8px] border border-[#E2E8F0]">
          ...
          <input v-model="searchQuery" class="w-full border-none outline-none text-[#425466]" placeholder="Search for an exercise" />
        </div>
  
        <div class="grid grid-cols-2 gap-[15px]">
          <div v-for="exercice in filteredExercises" :key="exercice.id" class="space-y-[15px] rounded-[16px] shadow p-[12px]">
            ...
            <Button 
              @click="addExercice(exercice)" 
              class="w-full" 
              :class="{'bg-green-500': isSelected(exercice), '': !isSelected(exercice)}" 
              :label="isSelected(exercice) ? 'Added' : 'Add Exercise'" 
              :primary="false" 
            />
          </div>
        </div>
  ...
  onMounted(() => {
    fetchData('exercises');
    const storedExercises = JSON.parse(localStorage.getItem('selectedExercises') || '[]');
    selectedExercises.value = storedExercises;
  });
